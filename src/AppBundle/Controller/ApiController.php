...
//    AZPIATALAK
     ...
     * @ApiDoc(
     *   resource = true,
     *   description = "Zerga ordenantzen historikoa eskuraty",
     *   statusCodes = {
     *     200 = "Zuzena denean"
     *   }
     * )
     ...
     *
     * @Annotations\View()
     * @Get("/historikoa/{udalkodea}")
     * @return View|Response
     */
    public function getHistorikoaAction($udalkodea)
    {
        $em         = $this->getDoctrine()->getManager();
        $historikoa = $em->createQueryBuilder()->select('h.id, h.bogargitaratzedata, h.bogbehinbetikodata, h.bogestekaes, h.bogestekaeu, h.fitxategia, h.indarreandata, h.onartzedata')
                    ->from('AppBundle:Historikoa', 'h')
                   ->innerJoin('h.udala', 'u')
                   ->where('u.kodea=:udalkodea')->setParameter('udalkodea', $udalkodea)
                   ->getQuery()
                   ->getResult();

        // $view = View::create();
        // $view->setData($historikoa);
        // $json = $this->get('jms_serializer')->serialize($historikoa, 'json');

        header('content-type: application/json; charset=utf-8');
        header('access-control-allow-origin: *');

        return new Response($this->get('jms_serializer')->serialize($historikoa, 'json'));
